...
describe('background platform strategy registry', () => {
  ...
  it('derives inline thread and API reply continuation policy from strategies', () => {
    ...
    expect(shouldUseInlineThread('x', true, image)).toBe(true);
    expect(shouldUseInlineThread('x', true, video)).toBe(false);
    expect(shouldUseInlineThread('x', false, video)).toBe(true);
